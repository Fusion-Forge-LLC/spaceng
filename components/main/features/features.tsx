...
function Features({title, note, image}: {title: string; note: string; image: string}) {
  return (
    <li className="shadow-[2px_0px_15px_rgba(0,0,0,0.25)] h-[400px] h rounded-[15px] flex gap-10 even:flex-row-reverse group">
      <article className="flex flex-col group-even:items-end gap-10 p-4 w-1/3 shrink-0">
        <h3 className="p-3 font-semibold  text-white px-8 features-text">{title}</h3>
        <p className="p-3 px-8 bg-white text-sm border border-grey-100 shadow-[2px_0px_15px_rgba(0,0,0,0.25)]">
          {note}
        </p>
      </article>
