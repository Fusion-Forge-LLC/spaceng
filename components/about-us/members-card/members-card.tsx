...
function MembersCard({name, role}: {name: string; role: string}) {
  return (
    <li className="bg-grey space-y-8 p-4">
      ...
      <div className="w-4/5">
        <h4 className="bg-white rounded-[15px] py-2 px-3 text-xl mb-4 font-medium">{name}</h4>
        <span
          className={cn(
            montesserat.className,
            "text-lg font-medium rounded-[15px] py-2 px-3 border border-white text-white block",
          )}
        >
          {role}
        </span>
